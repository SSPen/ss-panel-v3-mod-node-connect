...
Libtest(){
	...
	if [ "$libAddr" == "$GIT" ];then
		libAddr='https://raw.githubusercontent.com/SSPen/ss-panel-v3-mod-node-connect/master/libsodium-1.0.13.tar.gz'
	else
...
install_centos_ssr(){
	...
	ldconfig
	git clone -b master https://github.com/SSPen/shadowsocks.git "/root/shadowsocks"
	cd /root/shadowsocks
	...
	if [ -z "`python -c 'import requests;print(requests)'`" ]; then
		...
		git clone https://github.com/SSPen/urllib3.git && cd urllib3
...
install_node(){
	...
	echo "# Github: https://github.com/SSPen/ss-panel-v3-mod-node-connect   "
	...
	wget -N -P  /etc/ --no-check-certificate  https://raw.githubusercontent.com/SSPen/ss-panel-v3-mod-node-connect/master/supervisord.conf
	supervisord
	#iptables
	iptables -F
	iptables -X  
